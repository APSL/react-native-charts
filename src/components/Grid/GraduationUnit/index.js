...
export default class Grid extends Component {
  ...
  getStyles() {
    const { horizontal, labelWrapperFlex, unitFlex } = this.props
  ...
  getOrderedGraduationUnitValues() {
    const { horizontal, graduation, maxValue } = this.props

    const graduationUnitsCount = graduation > 0 ? Math.ceil(maxValue / graduation) : 1

    // Generate an iterable array of length `graduationUnitsCount` and map it
    // to graduation unit values...
    const graduationUnitValues = Array.apply(
      null,
      Array(graduationUnitsCount)
    ).map((value, index) => {
      return graduation * (index + 1)
    })

    return !horizontal ? graduationUnitValues.reverse() : graduationUnitValues
  }

  renderGraduationUnits() {
    const { horizontal, labelWrapperFlex, lineColor, unitFlex } = this.props
    ...
    return (
      <View style={this.getStyles().graduationUnits}>
        {this.getOrderedGraduationUnitValues().map((value, index) => (
          <GraduationUnit
            key={value}
            completeBorder={horizontal ? index >= lastGraduationUnitIndex : index === 0}
  ...
  renderContentContainer() {
    const { content, contentContainerStyle, horizontal } = this.props

    return (
      <View style={this.getStyles().contentContainerWrapper}>
        {horizontal
          ? null
          : <View style={this.getStyles().contentContainerLabelWrapperOffset} />}
